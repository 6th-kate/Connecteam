...
@Composable
fun LoadingGameInviteScreen(
    navController: NavController,
    inviteCode: String?,
    viewModel: LoadingGameInviteViewModel = hiltViewModel()
) {
    LaunchedEffect(viewModel.isLoading) {
        if (viewModel.isLoading) {
            viewModel.loadFromInviteCode(inviteCode)
        } else if (viewModel.error == null) {
            navController.navigate("${NavigationItem.GameInviteUnauth.route}/${viewModel.signInParams}") {
                popUpTo(0)
            }
        }
    }
